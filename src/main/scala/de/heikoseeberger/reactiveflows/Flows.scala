...
import de.heikoseeberger.reactiveflows.util.ShardingSupport
...
class Flows(protected val system: ExtendedActorSystem)
  ...
  def addMessage(
    name: String, text: Future[String]
  )(implicit timeout: Timeout, ec: ExecutionContext, sender: ActorRef): Future[Flow.MessageAdded] =
    text.flatMap(addMessage(name))
